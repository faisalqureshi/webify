...
I decided to go with markdown+pandoc combination.  From henceforth I will develop my course content in markdown, and I would use pandoc to convert my notes into the desired format (html, LaTeX articles, or a beamer slides).
...
Use the `preprocess-frontmatter` to control this behavior.  The default value for this key is `False`.
...
MDfile utility converts markdown files to html pages, LaTeX articles, or beamer slides using pandoc.  In addition to using pandoc templates during conversion, MDfile also supports mustache and jinja templating.  This allows the mdfile utility to generate complicated html pages by using information stored in external yaml files in addition to the contents of the markdown file.  Information stored in the external yaml files and the yaml front matter is available as the rendering context during the rendering process.  
...
These can be converted to LaTeX article, beamer slides, or html web page, respectively, using the following command: `python mdfile2.py filename.md`. Yaml front matter for the three files are shown below.
...
Use the following command to checkout various commandline options for mdfile.

```bash
$ python mdfile2.py --help
```
...
Information stored in external yaml files is available as the rendering context during the rendering operation.
